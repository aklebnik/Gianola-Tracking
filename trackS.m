function tracks = trackS(xyzs,mxdisp,param)
...
% Image Processing
% CALLING SEQUENCE:
% result = track( positionlist, mxdisp, param )
...
%  positionlist(1:d-1,:): contains the d coordinates and
...
% mxdisp: an estimate of the maximum distance that a particle 
...
% RESTRICTIONS:
% mxdisp should be set to a value somewhat less than the mean 
% spacing between the particles. As mxdisp approaches the mean
...
% by re-executing the function with a smaller value of mxdisp.
...
% a new position within mxdisp of an old position ( particle loss )
% penalize the total squared displacement by mxdisp^2. For non-
...
% ( provided mxdisp >> RMS displacement between frames ).
...
res = [1,res',length(t)];
ngood = res(2) - res(1) + 1; %number of points in first slides
eyes = 1:ngood;
pos = xyzs(eyes,1:dim);
istart = 2;
n = ngood;
...
notnsqrd = (sqrt(n*ngood) > 200) & (dim < 7); %1 if both are non-zero, 0 otherwise
notnsqrd = notnsqrd(1);
...
if notnsqrd
    ...
    %   calculate a blocksize which may be greater than mxdisp, but which
    ...
    volume;
    blocksize = max( [mxdisp,((volume)/(20*ngood))^(1.0/dim)] );
end
%%
...
for i=istart:z
    ...
    m = res(i+1) - res(i); %number of new centers
    res(i);
    eyes=1:m;
    eyes = eyes+ res(i); %index of all new points
    
    if m > 0 %if there are new centers
        
        xyi = xyzs(eyes,1:dim); %takes the relevant parts of the big matrix
        found = zeros(m,1); %assume no particles will be found
        
        %%
        % THE TRIVIAL BOND CODE BEGINS   
  
        if notnsqrd %if there are tons of particles, use this method
            ...
            abi = fix(xyi./blocksize); %the speciic stack, divided by a value, then turned to an integer
       %     abi = abi (1:end-1, :);
            abpos = fix(pos./blocksize); %same as above
            si = zeros(m,1); %inital values
            spos = zeros(n,1);
            dimm = zeros(dim,1);
            coff = 1.;
            
            for j=1:dim %for each dimension
                minn = min([abi(:,j);abpos(:,j)]); %find the smallest value
                maxx = max([abi(:,j);abpos(:,j)]); %find the largest value
                abi(:,j) = abi(:,j) - minn; %subtract off the minimum value
                abpos(:,j) = abpos(:,j) - minn; %abi and abpos are equivelent here....
                dimm(j) = maxx-minn + 1; %create a new value, based on the difference in values
                si = si + abi(:,j).*coff; % for each particle, multiply subsequent dimensions by  
                spos = spos + abpos(:,j).*coff;
                coff = dimm(j).*coff; %increase the coefficient?????
            end
            nblocks = coff;
...
%%
        else  %if less than 200 centers in slide, do this method
...
%%
if goodenough > 0  %check if you got useful values
