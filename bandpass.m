function res = bandpass(image_array,lnoise,dim,plots,lobject,threshold)
% 
% NAME:
%               bandpass
% PURPOSE:
...
%                       performed.
%               dim:  Dimensions of image, in pixels
%               plots:  Boolean of displaying plots? 1 is yes.
...
%
%               Modified code to suit DSG group AK 6/14
%
...
if plots==1
    ...
    title ('pre bandpass, (a)')
    subplot (2,2,2)
    colormap ('gray'), image(res);
    title('post bandpass, (b)')
    if min(dim)>= 51 %if box is over 50 pixels, plot this
        subplot (2,2,3)
        colormap ('gray'), imagesc(res(1:50, 1:50))
        if min(dim) >= 151
            subplot (2,2,4)
            colormap ('gray'), imagesc(res(1:150, 1:150))
        end
    end
end
